from flask import (
    ...
    session, flash, send_file, after_this_request
)
...
def api_mes_total_geral():
    ...
    for mes in MESES_ORDEM:
        ...
        media_percent_mes = round(soma_css_peso_mes / soma_css_mes, 1) if soma_css_mes > 0 else 0
    ...
    media_percent_anual = round(soma_css_peso_anual / soma_css_anual, 1) if soma_css_anual > 0 else 0
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            for url in urls:
                try:
                    ...
                    resultado.append({
                        "url": url,
                        "erro": "Não foi possível obter informações"
                    })
...
def download_video():
    ...
    if not os.path.exists(caminho):
        return jsonify({"erro": "Arquivo não foi criado"}), 500
